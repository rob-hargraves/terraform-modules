...
resource "aws_cloudwatch_metric_alarm" "unauthorized_api_calls" {
  ...
  metric_name         = "${aws_cloudwatch_log_metric_filter.unauthorized_api_calls.metric_transformation["name"]}"
  namespace           = "${aws_cloudwatch_log_metric_filter.unauthorized_api_calls.metric_transformation["namespace"]}"
  period              = "300"
  statistic           = "Sum"
  threshold           = "1"
}
